...
export const simulateMarginPosition = async () => {
  ...
  await ctx.sudo(
    ctx.api.tx.marginProtocol.setTradingPairRiskThreshold(
      ['FEUR', 'AUSD'],
      { margin_call: 1_000_000, stop_out: 0 },
      { margin_call: 990_000, stop_out: 0 },
      { margin_call: 990_000, stop_out: 0 }
    )
  ).send;
  ...
  await ctx.sudoAs(
    trader,
    ctx.api.tx.marginProtocol.openPosition(0, ['FEUR', 'AUSD'], 'LongTwo', dollar(100), dollar(1))
  ).finalized;
